...
“初始化”和“清除”是這些安全問題的其中兩個。許多C程序的錯誤都是由於程式開發人員忘記初始化一個變量造成的。對於現成的函式庫，若用戶不知道如何初始化函式庫的一個組件，就往往會出現這一類的錯誤。清除是另一個特殊的問題，因為用完一個元素後，由於不再關心，所以很容易把它忘記。這樣一來，那個元素佔用的資源會一直保留下去，極易產生資源（主要是內存）用盡的後果。
