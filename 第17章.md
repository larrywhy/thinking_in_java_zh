...
頭兩個類是“支持／工具”類，作用是使程序剩餘的部分在編寫時更加連貫，也更便於閱讀。第一個是`Pr`，它類似ANSI C的`perror`函式庫，兩者都能打印出一條錯誤提示消息（但同時也會退出程序）。第二個類將文件的創建過程封裝在內，這個過程已在第10章介紹過了；大家已經知道，這樣做很快就會變得非常累贅和麻煩。為解決這個問題，第10章提供的方案致力於新類的創建，但這兒的“靜態”方法已經使用過了。在那些方法中，正常的異常會被捕獲，並相應地進行處理。這些方法使剩餘的代碼顯得更加清爽，更易閱讀。
...
程序列表剩下的部分由`main()`構成，它負責控制命令行參數，並判斷我們是準備在標準Java函式庫的基礎上構建由一系列類名構成的“倉庫”，還是想檢查已寫好的那些代碼的正確性。不管在哪種情況下，都會創建一個`ClassScanner`對象。
