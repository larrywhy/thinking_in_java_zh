...
此時最直接的一個解決辦法就是乾脆不這樣做：不要有意將多個引用指向同一個作用域內的同一個對象。這樣做可使程式碼更易理解和調試。然而，一旦準備將引用作為一個變數或參數傳遞——這是Java設想的正常方法——別名問題就會自動出現，因為創建的本地引用可能修改“外部對象”（在方法作用域之外創建的對象）。下面是一個例子：
...
`DepthReading`和`TemperatureReading`非常相似；它們都只包含了基本數據類型。所以`clone()`方法能夠非常簡單：調用`super.clone()`並返回結果即可。注意兩個類使用的`clone()`程式碼是完全一致的。
